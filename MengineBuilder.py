...
def _check_runtime():
    print("Mengine-Builder Python: %s (%s)" % (sys.executable, platform.python_version()))

    if sys.version_info < MINIMUM_PYTHON:
        print(
            "Mengine-Builder requires Python %d.%d or newer. Install/activate a compatible Python."
            % MINIMUM_PYTHON,
            file=sys.stderr,
        )
        ...
        if sys.platform == "darwin":
            ...
            print("Then run: /opt/homebrew/bin/python3.14 MengineBuilder.py ...", file=sys.stderr)
    ...
    try:
        ...
        print("Mengine-Builder requires Pillow. Install it with: %s" % _dependency_hint(), file=sys.stderr)
        return False
...
def main(arguments=None):
    ...
    if help_requested is False:
        try:
            ...
            print("Mengine-Builder preflight failed: %s" % exception, file=sys.stderr)
            return 2
